...
# %% Calcolo delle Medie Mobili (su una finestra di 3 anni)
df_2010_2024['Media_mobile_maschi'] = (df_2010_2024['Totale maschi'].rolling(window=3, min_periods=1).mean())
df_2010_2024['Media_mobile_femmine'] = (df_2010_2024['Totale femmine'].rolling(window=3, min_periods=1).mean())

# %% Calcolo del Rapporto Maschi/Femmine
df_2010_2024['Rapporto_maschi_femmine'] = (df_2010_2024['Totale maschi'] / df_2010_2024['Totale femmine'])
df_2010_2024.head()
...
for anno in range(2025, 2041):
    ...
    df_2024['Media_mobile_maschi'] = (df_2024['Totale maschi'].rolling(window=3, min_periods=1).mean())
    df_2024['Media_mobile_femmine'] = (df_2024['Totale femmine'].rolling(window=3, min_periods=1).mean())

    df_2024['Rapporto_maschi_femmine'] = (df_2024['Totale maschi'] / df_2024['Totale femmine'])

    X = df_2024[['Anno', 'Età', 'Totale maschi -1', 'Totale femmine -1',
                 'Totale -1', 'Tasso_crescita_maschi', 'Tasso_crescita_femmine',
                 'Media_mobile_maschi', 'Media_mobile_femmine', 'Rapporto_maschi_femmine']].values
...
df_2025_2040.to_csv('../previsioni/Test7_Previsione_Popolazione_2025-2040.csv', index=False)
print(df_2025_2040)
